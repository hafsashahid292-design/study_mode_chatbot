...
st.markdown(
    ...
    /* Sidebar (matching main background) */
        ...
        try:
            ...
            for chunk in response:
                if hasattr(chunk, "text") and chunk.text:
                    ...
                    placeholder.markdown(full_text + "▌")  # only while streaming

            # Final clean render (remove trailing ▌ or whitespace)
            if full_text:
...
if feature_mode == "📝 Quiz Generator":
    ...
    if st.session_state.quiz_generated and st.session_state.last_quiz:
        followup_prompt = st.chat_input("Ask a question about this quiz...")
        if followup_prompt:
...
# --- Normal Chat for Other Features ---
    ...
    user_prompt = st.chat_input("Ask me a question about your studies...")
    if user_prompt:
        with st.chat_message("user"):
